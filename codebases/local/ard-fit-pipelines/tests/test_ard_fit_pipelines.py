...
def test_ColumnsTransform(diamonds_df):
    ...
    assert (
        t.fit(df, bindings={'which_cols': ['x', 'y', 'z']}).apply(df)
        .equals(df[['x', 'y', 'z']])
    )
    ...
    assert (
        fpc.hp_cols(cols=['x', 'y', 'z']).resolve(bindings)
        == ['x', 'y', 'z']
    )
    assert (
        fpc.hp_cols(cols=['x', fpc.hp('some_col'), 'z']).resolve(bindings)
        == ['x', 'y', 'z']
    )
    assert (
        fpc.hp_cols(cols=['x', '{some_col}', 'z']).resolve(bindings)
        == ['x', 'y', 'z']
    )
...
def test_CopyColumns(diamonds_df):
    ...
    result = fpt.CopyColumns(['price'], ['price_copy1',
        'price_copy2']).apply(df)
    ...
    with pytest.raises(ValueError):
        result = fpt.CopyColumns(['price', 'x'], ['price_copy',]).apply(df)
